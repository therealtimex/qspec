...
const { basename, dirname, isAbsolute, join, normalize, relative, resolve } = require("node:path");
...
function entry(root, file, { kind, id, instance_version, status, fingerprint, findings, recordFile, detail, rendered, output }) {
  ...
  if (output) out.output = relative(root, resolve(output));
...
function write(root, command, entries, label = null) {
  ...
  for (const e of entries) {
    ...
    if (e.rendered != null) {
      ...
      const name = `${(e.output ?? e.path).replace(/[\\/]/g, "_").replace(/\.(?:ya?ml|md)$/, "")}.md`;
...
function includesSpec(run, root, spec) {
  if (!spec) return true;
  const wanted = String(spec);
  const paths = new Set([
    normalize(wanted),
    normalize(relative(root, resolve(wanted))),
    normalize(relative(root, resolve(root, wanted))),
  ]);
  return run.record.files.some((f) => f.kind === "spec" && (f.id === wanted || paths.has(normalize(f.path))));
}

// Every recorded run, oldest first. A spec id or path scopes the history to
// runs that actually included that spec; labels are intentionally not global.
function listing(root, spec = null) {
  const base = join(root, RUNS);
  ...
  return out
    .sort((x, y) => (x.record.recorded < y.record.recorded ? -1 : x.record.recorded > y.record.recorded ? 1 : x.name < y.name ? -1 : 1))
    .filter((r) => includesSpec(r, root, spec));
}
...
// The runs and attached note bodies for one spec, ready for a dossier. Reading
// happens here, beside the storage contract, so the renderer receives the note
// text whole and never has to infer where it lives.
function history(root, spec) {
  return listing(root, spec).map((run) => ({
    ...run,
    notes: (run.record.notes ?? []).map((note) => {
      let text;
      try { text = readFileSync(join(root, RUNS, run.name, note.path), "utf8"); }
      catch { text = "(note file missing)"; }
      return { ...note, text };
    }),
  }));
}
...
function load(root, name, spec = null) {
  const all = listing(root, spec);
...
function attach(root, runName, file, { actor, role, kind = "note" }) {
  ...
  const label = String(run.record.label ?? "").toLowerCase().replace(/[^a-z0-9]+/g, "-").replace(/^-|-$/g, "");
  const roleLabel = slug(role);
  const followsRoleConvention = /^[a-z0-9]+(?:-[a-z0-9]+)+-round-[0-9]+$/.test(label);
  const warning = followsRoleConvention && !new RegExp(`-${roleLabel}-round-[0-9]+$`).test(label)
    ? `warning: run ${run.name} has label '${run.record.label}', which does not match --role '${String(role).trim()}'; lint under your own <spec>-<role>-round-<n> label before attaching`
    : null;
  ...
  return { run: run.name, note, warning };
}
...
function show(root, runName, spec = null) {
  const run = load(root, runName, spec);
  const r = run.record;
  ...
  for (const f of r.files) {
    ...
    if (f.output) lines.push(`      output: ${f.output}`);
...
module.exports = { RUNS, KINDS, entry, write, listing, latest, load, diff, sourceDiff, summary, attach, notesSince, show, history };
