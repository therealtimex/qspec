...
function parseBib(text) {
  ...
  const enclosed = (open, close, start, quotedValues = true) => {
    let depth = 1;
    let braces = 0;
    let quoted = false;
    let escaped = false;
    while (i < text.length) {
      ...
      const insideBraces = open === "{" ? depth > 1 : braces > 0;
      if (c === '"' && quotedValues && (quoted || !insideBraces)) {
        quoted = !quoted;
        continue;
      }
      if (quoted) continue;
      if (open === "(" && c === "{") { braces++; continue; }
      if (open === "(" && c === "}" && braces > 0) { braces--; continue; }
  ...
  try {
    while (i < text.length) {
      ...
      if (specialTypes.has(type)) {
        enclosed(open, close, entryStart, type !== "comment");
        continue;
      }
