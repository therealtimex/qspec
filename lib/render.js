...
const { catalogs, profileFieldLabel, resolveProfile, valueLabel } = require("./catalogs.js");
...
const profileValue = (v) => {
  if (Array.isArray(v)) return v.length ? v.map((item) => valueLabel(item) ?? item).join("; ") : hole;
  const token = s(v == null ? null : String(v));
  return token ? valueLabel(token) ?? token : hole;
};
...
function committeeContext(spec) {
  ...
  switch (spec.domain) {
    case "social":
      return `${why}\n\nThe question concerns ${show(c.object)}. Its scope is ${show(c.scope)}.`;
    case "natural":
      return `${why}\n\nThe question examines ${show(c.object)} in ${show(c.system)}. Its scope is ${show(c.scope)}.`;
    case "engineering":
      return `${why}\n\nThe question concerns ${show(c.artifact_or_process)} in ${show(c.system)}, under ${show(c.operating_regime)}. It is assessed by ${show(c.metric)}.`;
    default:
      return why;
  }
}
...
function draftHoles(spec, definition) {
  ...
  const sf = spec.success_and_failure ?? {}, ask = spec.ask ?? {};
  const domain = catalogs.domains[spec.domain];
  const rows = [
    ["Research question title", spec.title], ["Owner", spec.owner], ["Reviewer", spec.reviewers],
    ["Claim", c.one_sentence], ["Why it matters", c.why_it_matters],
    ["Design", qt.method_family], ["Knowledge goal", qt.knowledge_goal],
    ["What this study would add", inc.increment_if_this_works],
    ["What would count as support", sf.support_would_look_like],
    ["What would count as failure", sf.failure_would_look_like],
    ["When the study stops", sf.kill_condition],
    ["Time requested", ask.time], ["People requested", ask.people],
    ["Access requested", ask.access], ["Hardware or compute requested", ask.hardware_or_compute],
  ];
  const contextLabels = {
  ...
  for (const field of domain?.claim_required ?? []) rows.push([contextLabels[field] ?? words(field), c[field]]);
  for (const conditional of domain?.claim_conditional ?? []) {
    if (!(conditional.unless_family ?? []).includes(qt.method_family)) rows.push([contextLabels[conditional.field] ?? words(conditional.field), c[conditional.field]]);
  }
  const closest = Array.isArray(inc.closest_work) ? inc.closest_work : [];
  if (!closest.length) rows.push(["Closest work", null]);
  for (const [i, work] of closest.entries()) {
    rows.push([`Closest work ${i + 1}`, work.cite]);
    rows.push([`What closest work ${i + 1} establishes`, work.settled]);
    rows.push([`What closest work ${i + 1} leaves open`, work.still_open]);
  }
  for (const field of definition?.required ?? []) rows.push([profileFieldLabel(field) ?? words(field), spec.profile?.[field]]);
  const missing = rows.filter(([, v]) => Array.isArray(v) ? !v.length : !s(v == null ? null : String(v))).map(([label]) => label);
...
function sheet(spec, { record = null, index = null, draft = false } = {}) {
  ...
  const profileRows = definition ? [...definition.required, ...definition.optional]
    ...
    .map((field) => `**${profileFieldLabel(field) ?? words(field)}:** ${profileValue(spec.profile?.[field])}`) : [];
  const obtainable = Array.isArray(mat.obtainable) ? mat.obtainable : [];
  const materialLines = [
    "**In hand:**", "", list(mat.in_hand), "", "**Still needed:**", "",
    ...(Array.isArray(mat.blocking) && mat.blocking.length ? mat.blocking.map((item) => `- ${item}.`) : ["- No blocking material is recorded."]),
    ...obtainable.map((item) => `- ${show(item.item)}. Source: ${show(item.source)}. Expected horizon: ${show(item.horizon)}. Likelihood: ${humanValue(item.probability)}.`),
    "", `**Access risk:** ${humanValue(mat.access_risk)}.`, "",
  ];
  const safety = Array.isArray(con.safety_or_ethics) && con.safety_or_ethics.length
    ? con.safety_or_ethics.map((token) => `${humanValue(token)}.`).join(" ")
  ...
  const md = head("Research question", spec, headRows) + [
    ...
    "### Design", "", `The design is ${humanValue(qt.method_family)}, with ${humanValue(qt.knowledge_goal)} as its knowledge goal.`, "",
    ...profileRows, ...(profileRows.length ? [""] : []),
    ...(qt.secondary_method ? [`**Secondary design:** ${humanValue(qt.secondary_method)}.`, `**Rescue rule:** ${show(qt.rescue_rule)}`, ""] : []),
    "### What is known and what is still open", "",
    ...(closest.length ? closest.map((work) => `- ${cited(work)}. **What it establishes:** ${show(work.settled)} **What remains open:** ${show(work.still_open)}`) : [`- ${hole}`]), "",
    ...
    ...(entry ? ["### Recommended action", "", `The round recommends: ${humanValue(entry.recommended_action)} (rank ${entry.rank}).`, ""] : []),
