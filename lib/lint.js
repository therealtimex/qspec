...
function lintSpec(spec, file = "<spec>") {
  ...
  if (Object.prototype.hasOwnProperty.call(mat, "obtainable") && !isList(mat.obtainable)) f("M9", "materials.obtainable must be a list");
  const optionalMaterialFields = (entry, path) => {
    if (entry?.access != null && !catalogs.core.access.includes(entry.access)) {
      f("M9", `${path}.access must be one of ${catalogs.core.access.join(", ")}`);
    }
    for (const k of ["locator", "coverage", "evidence"]) {
      if (entry?.[k] != null && !nonEmpty(entry[k])) f("M9", `${path}.${k} must be a non-empty string when supplied`);
    }
  };
  (isList(mat.blocking) ? mat.blocking : []).forEach((entry, i) => {
    if (typeof entry === "string") {
      if (!nonEmpty(entry)) f("M9", `materials.blocking[${i}] empty`);
      return;
    }
    if (!entry || typeof entry !== "object" || !nonEmpty(entry.item)) f("M9", `materials.blocking[${i}].item empty`);
    optionalMaterialFields(entry, `materials.blocking[${i}]`);
    if (entry?.horizon != null && !nonEmpty(entry.horizon)) f("M9", `materials.blocking[${i}].horizon must be a non-empty string when supplied`);
    if (entry?.probability != null && !catalogs.core.levels.includes(entry.probability)) f("M9", `materials.blocking[${i}].probability must be low, medium, or high when supplied`);
  });
  (isList(mat.obtainable) ? mat.obtainable : []).forEach((o, i) => {
    ...
    optionalMaterialFields(o, `materials.obtainable[${i}]`);
  });
  if (isList(mat.blocking) && mat.blocking.length > 0 && !(isList(mat.obtainable) ? mat.obtainable : []).length) f("blocking-without-plan", `${mat.blocking.length} blocking material(s) and no obtainable entry naming a source and horizon`, "warn");
  const needed = [
    ...(isList(mat.blocking) ? mat.blocking : []).map((entry, i) => ({ entry: typeof entry === "string" ? { item: entry } : entry, path: `materials.blocking[${i}]` })),
    ...(isList(mat.obtainable) ? mat.obtainable : []).map((entry, i) => ({ entry, path: `materials.obtainable[${i}]` })),
  ];
  for (const { entry, path } of needed) {
    if (!entry || typeof entry !== "object") continue;
    if (!nonEmpty(entry.locator) && !nonEmpty(entry.evidence)) {
      f("material-unlocated", `${path} '${entry.item ?? "(unnamed)"}' has neither locator nor evidence`, "warn", "locate the material and record where it is or how its existence is known");
    }
    if (nonEmpty(entry.horizon) && (!nonEmpty(entry.access) || entry.access === "unknown")) {
      f("horizon-without-access", `${path}.horizon is stated but access is ${nonEmpty(entry.access) ? `'${entry.access}'` : "not stated"}`, "warn", "identify the access mode before stating an expected horizon");
    }
  }
