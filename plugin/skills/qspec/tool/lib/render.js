...
// Render those placeholders with single angle quotation marks, including when
// the author put one in code, while preserving real HTML wherever it appears.
...
function head(kind, spec, rows) {
  return [`# ${kind}`, `## ${spec.title}`, ...rows.map(([k, v]) => `**${k}:** ${show(v == null ? null : String(v))}`), "", "---", "", ""].join("\n");
}
...
function index(idx, resolved = null) {
  ...
  if (!s(idx.decision_maker)) f("manual", "index-decision-maker", "the Index names no decision-maker, so a freeze cannot be bound to its committee", "fill `decision_maker:` in the Index");
  ...
  const md = [
    `# Selection round ${idx.round}`, `**Date:** ${show(idx.date == null ? null : String(idx.date))}`, `**Decision-maker:** ${show(idx.decision_maker == null ? null : String(idx.decision_maker))}`, "", "---", "", "",
