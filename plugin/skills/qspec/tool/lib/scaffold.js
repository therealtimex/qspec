...
const { citationCounts, readBib } = require("./bib.js");
...
const REFERENCES = `% Bibliography for the closest-work citations in this QSPEC project.
% qspec never writes an entry; add each one from publisher or DOI metadata.
`;
...
{invocation} sheet specs/<Q-id>_<slug>.yaml --index specs/index-{round}.yaml --out sheets/<Q-id>.md   # committee document
{invocation} dossier specs/<Q-id>_<slug>.yaml --out documents/dossiers/<Q-id>.md   # internal process record
{invocation} request specs/<Q-id>_<slug>.yaml --out requests/<Q-id>.md   # frozen only; the Paperforge handoff
{invocation} render --out documents/                   # committee sheets and Index, unpublished dossiers, frozen requests
{invocation} render --draft --out documents/           # unsigned owner previews under drafts/
...
- A Selection Sheet is for a committee. A dossier is for the people inside the
  process. Declare it as a Paperforge \`qspec-dossier\` document with
  \`publish = false\`; its cover identifies it as an internal process record.
  Bare note placeholders such as \`<run>\` render as \`‹run›\`; source notes stay
  untouched and all three dossier editions remain safe to verify.
  Run \`qspec render --out documents/\`, then \`paperforge all --draft --config
  documents/documents.toml\` to build and verify every unpublished document.
...
function create(directory, { title, round, decisionMaker, brief, domain, append = false, git = true, invocation = "qspec" } = {}) {
  ...
  if (existsSync(join(root, "references.bib"))) throw new Error(`${join(root, "references.bib")} exists; init never overwrites a writer-owned bibliography`);
...
function survey(specsDir) {
  ...
  for (const f of names) {
    ...
    if (kind === "index") rounds.push(f);
      ...
      byStatus[doc.status ?? "(no status)"] = (byStatus[doc.status ?? "(no status)"] ?? 0) + 1;
      specs.push(doc);
    }
    else unreadable++;
  }
  return { byStatus, rounds: rounds.sort(), records, specs, unreadable };
}
...
function doctor({ project, invocation = "qspec", cwd = process.cwd() } = {}) {
  ...
  if (!existsSync(bibliography)) {
    lines.push(`  ${"bibliography".padEnd(10)} ${"none".padEnd(8)} references.bib at the project root holds the entries the specs cite`);
  } else {
    const bib = readBib(bibliography);
    if (bib.error) lines.push(`  ${"bibliography".padEnd(10)} ${"skip".padEnd(8)} references.bib cannot be parsed at line ${bib.error.line}: ${bib.error.message}`);
    else {
      const counts = citationCounts(s?.specs ?? [], bib.entries);
      lines.push(`  ${"bibliography".padEnd(10)} ${String(bib.entries.size).padEnd(8)} ${counts.unresolved} unresolved, ${counts.unkeyed} unkeyed closest-work citation(s) across ${(s?.specs ?? []).length} spec(s)`);
    }
  }
