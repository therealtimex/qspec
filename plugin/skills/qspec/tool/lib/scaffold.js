...
const COMMANDS = ["init", "new", "doctor", "lint", "fingerprint", "sign", "transition", "sheet", "index", "request", "paper"];
...
function stamp(root, facts) {
  ...
  writeFileSync(path, JSON.stringify({ version: VERSION, agents: fingerprint(), created: new Date().toISOString().replace(/\.\d{3}Z$/, "Z"), ...facts }, null, 2) + "\n");
  return path;
}
...
function create(directory, { title, round, decisionMaker, brief, domain, append = false, git = true, invocation = "qspec" } = {}) {
  ...
  if (existsSync(join(root, STAMP))) throw new Error(`${root} is already a QSPEC project (${STAMP} exists); run \`qspec doctor\` there`);
...
function doctor({ project, invocation = "qspec", cwd = process.cwd() } = {}) {
  ...
  if (found.state === "stale") { problems++; lines.push(`  ${"".padEnd(10)} ${"".padEnd(8)} compare AGENTS.md against a fresh \`qspec init\` and copy across what is missing`); }
  const agentsPath = join(root, "AGENTS.md");
...
module.exports = { COMMANDS, STAMP, MARK_OPEN, MARK_CLOSE, fingerprint, drift, findRoot, create, newSpec, doctor, titleFrom };
