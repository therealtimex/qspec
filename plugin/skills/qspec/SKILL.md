...
```bash
node <skill-dir>/tool/bin/qspec.mjs help
node <skill-dir>/tool/bin/qspec.mjs lint specs/Q-014_apical-oxygen.yaml
```
...
3. **Signing is a person's act.** When lint is clean, tell the user which reviewer must reread the spec and run `qspec sign --by <reviewer>`. Do not sign as the reviewer yourself unless the user, acting as that reviewer, says the invariants hold.
4. **Offer, choose, freeze** with `transition`, and render the Selection Sheet for the decision-maker. One freeze per round unless the Index writes an exception.
5. **Hand off** with `request`. Point the Paperforge project's `request` key at the exported file and put `**Question:** Q-014@1` in the document head.
...
**Never edit a Decision Record by hand.** It is append-only and tool-written. If a state is wrong, append the right transition.
