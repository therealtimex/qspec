...
node -e 'const a=require("node:assert/strict"),y=require("./lib/vendor/js-yaml/js-yaml.js"),fs=require("node:fs"),r=require("./lib/render.js"); const i=y.load(fs.readFileSync("examples/index-round-2026-09.yaml","utf8"),{schema:y.CORE_SCHEMA}); i.decision_maker=""; const out=r.index(i); a.ok(out.md.includes("**Decision-maker:** (not stated)")); a.deepEqual(out.findings,[{severity:"manual",rule:"index-decision-maker",message:"the Index names no decision-maker, so a freeze cannot be bound to its committee",act:"fill `decision_maker:` in the Index"}])'
...
node -e 'const a=require("node:assert/strict"),fs=require("node:fs"); const note=fs.readFileSync(process.argv[1],"utf8"),dossier=fs.readFileSync(process.argv[2],"utf8"),restored=dossier.replaceAll("‹","<").replaceAll("›",">"); a.ok(dossier.startsWith("# Process record (internal)\n")); a.ok(dossier.includes("‹run›")); a.ok(dossier.includes("‹handoff.md›")); a.ok(dossier.includes("`qspec runs --diff` ‹a›,‹b›")); a.ok(dossier.includes("Use ‹run›,<br>then attach the review.")); a.ok(dossier.includes("literal `‹run›` beside `materials.obtainable[*].horizon: \"\"`")); a.ok(dossier.includes("Rates <10% and >55%; see <https://example.invalid>; mail <reviewer@example.invalid>.")); a.ok(!dossier.includes("&lt;") && !dossier.includes("&gt;")); a.ok(!dossier.includes("`<run>`") && !dossier.includes("`<handoff.md>`")); a.ok(restored.includes(note))' "$NOTE" /tmp/qspec-render/Q-102-dossier.md || { echo "UNEXPECTED: dossier note did not disambiguate CLI metavariables from HTML, comparisons, and autolinks"; exit 1; }
node -e 'const a=require("node:assert/strict"),y=require("./lib/vendor/js-yaml/js-yaml.js"),fs=require("node:fs"),r=require("./lib/render.js"),s=y.load(fs.readFileSync("examples/ss-causal-procurement-cutoff.yaml","utf8"),{schema:y.CORE_SCHEMA}),md=r.dossier(s,{history:[{name:"probe",notes:[{kind:"review",actor:"A",role:"reviewer",attached:"2026-09-05",text:"Use <run>,<br>then attach; use <run>, <b>bold text</b>. Compare <a>,<b>, then keep <strong>real HTML</strong>, <a href=\"#detail\">a link</a>, and already-coded `<run>` as written.\n"}]}]}).md; a.ok(md.includes("Use ‹run›,<br>then attach")); a.ok(md.includes("use ‹run›, <b>bold text</b>")); a.ok(md.includes("Compare ‹a›,‹b›")); a.ok(md.includes("<strong>real HTML</strong>")); a.ok(md.includes("<a href=\"#detail\">a link</a>")); a.ok(md.includes("already-coded `‹run›`")); a.ok(!md.includes("``<run>``"))'
...
echo "== labelled body fields are paragraphs and array-valued profile fields are lists"
node -e 'const a=require("node:assert/strict"),fs=require("node:fs"); const paths=["/tmp/qspec-render/documents/sheets/Q-101.md","/tmp/qspec-render/documents/dossiers/Q-101.md","/tmp/qspec-render/documents/requests/Q-201.md","/tmp/qspec-render/documents/index/2026-09.md"]; for(const p of paths){const lines=fs.readFileSync(p,"utf8").split("\n"),body=lines.indexOf("---"); a.ok(body>=0,`${p}: missing head delimiter`); for(let i=body+1;i<lines.length;i+=1) if(/^\*\*[^*]+:\*\*/.test(lines[i])) a.equal(lines[i+1],"",`${p}:${i+1}: labelled body field is not followed by a blank line`)} for(const p of paths.slice(0,2)){const lines=fs.readFileSync(p,"utf8").split("\n"),i=lines.findIndex((line)=>/^\*\*(?:Pre-committed|Precommitted) checks:\*\*$/.test(line)); a.ok(i>lines.indexOf("---"),`${p}: pre-committed checks are not a labelled list`); a.equal(lines[i+1],""); a.equal(lines[i+2],"- Density test at cutoff"); a.equal(lines[i+3],"- Covariate continuity for buyer type and region"); a.equal(lines[i+4],"- Bandwidth sensitivity"); a.ok(!lines.slice(i,i+5).join("\n").includes(";"),`${p}: pre-committed checks were semicolon-joined`) }'
...
sed -n '/^\[types.qspec-dossier\]$/,/^$/p' templates/documents.qspec.toml | grep -q '^narrow_layout = false$' || { echo "UNEXPECTED: dossier verification is not scoped to wide layouts"; exit 1; }
...
echo "== render --manifest keeps a warning when a later edition is unpublished"
cp test/fixtures/publishable-dossier-multiedition.toml /tmp/qspec-render/documents/publishable-dossier-multiedition.toml
MULTIEDITION_SHA=$(shasum -a 256 /tmp/qspec-render/documents/publishable-dossier-multiedition.toml | awk '{print $1}')
$Q render --specs /tmp/qspec-render/specs --out /tmp/qspec-render/documents --index /tmp/qspec-render/specs/index-2026-09.yaml --manifest /tmp/qspec-render/documents/publishable-dossier-multiedition.toml --label publish-multiedition-warning-probe > /tmp/qspec-render/publish-multiedition-warning.out
grep -q "warning: dossier document 'qspec-dossier-multiedition' has publish = true; process records must remain unpublished" /tmp/qspec-render/publish-multiedition-warning.out || { echo "UNEXPECTED: a later unpublished edition suppressed the dossier warning"; exit 1; }
test "$MULTIEDITION_SHA" = "$(shasum -a 256 /tmp/qspec-render/documents/publishable-dossier-multiedition.toml | awk '{print $1}')" || { echo "UNEXPECTED: render edited the multi-edition Paperforge manifest"; exit 1; }
