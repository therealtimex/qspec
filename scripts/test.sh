...
node -e 'const a=require("node:assert/strict"),fs=require("node:fs"); const note=fs.readFileSync(process.argv[1],"utf8"),dossier=fs.readFileSync(process.argv[2],"utf8"),restored=dossier.replaceAll("‹","<").replaceAll("›",">"); a.ok(dossier.startsWith("# Process record (internal)\n")); a.ok(dossier.includes("‹run›")); a.ok(dossier.includes("‹handoff.md›")); a.ok(dossier.includes("Rates <10% and >55%; see <https://example.invalid>.")); a.ok(!dossier.includes("&lt;") && !dossier.includes("&gt;")); a.ok(!dossier.includes("`<run>`") && !dossier.includes("`<handoff.md>`")); a.ok(restored.includes(note))' "$NOTE" /tmp/qspec-render/Q-102-dossier.md || { echo "UNEXPECTED: dossier note did not protect placeholders while preserving comparisons and autolinks"; exit 1; }
